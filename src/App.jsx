      ...
      <main className="py-8 max-w-lg mx-auto">
        {route === '/' && (
          <div className="text-center space-y-4">
            ...
            <p className="text-xs text-silver">Market digital dengan layanan cepat, praktis, dan harga terjangkau.</p>
            <button onClick={() => { setSelected({name: 'PROMO WEB', price: 10000}); setRoute('/payment'); }} className="px-6 py-3 rounded-xl btn-custom text-xs font-bold w-full">[ 📦 PESAN DAN BAYAR ]</button>
          </div>
        )}
        ...
        {route === '/contact' && (
          <div className="text-center space-y-3">
            ...
            <a href={APP_CONFIG.SOCIALS.WHATSAPP_CHANNEL} target="_blank" rel="noreferrer" className="block w-full py-3 rounded-xl btn-custom text-xs font-bold">[ 📢 MASUK CHANNEL ]</a>
          </div>
        )}
        ...
        {route === '/payment' && selected && (
          <div className="glass-morphism p-6 rounded-2xl text-center space-y-4">
            ...
            <button onClick={() => setRoute('/')} className="block mx-auto text-xs text-silver mt-2">← Kembali</button>
          </div>
        )}
      </main>
    </div>
